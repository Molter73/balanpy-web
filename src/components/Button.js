...
const Button = ({ styles, text, href }) => {
  return (
    <Link href={href}>
      <button
        type="button"
        className={`py-2 px-4 font-poppins font-medium rounded-[5px] outline-none border-4 border-balanpy box-border transition-all duration-300 ease-in-out ${styles}`}
      >
        {text}
      </button>
    </Link>
